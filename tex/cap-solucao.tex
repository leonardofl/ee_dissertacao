...
Neste capítulo, apresentaremos a arquitetura e aspectos de implementação do \ee.   
...
chamada \textsf{Invoker}, que foi usada de forma disciplinada pelo projeto \gerosa{?}.
...
que utilizem o histórico de execução dos \emph{invokers} \gerosa{frase com estrutura confusa}.
...
da coreografia.
Mais detalhes sobre a implementação da garantia de idempotência, ver a Seção~\ref{sec:implementacao}.
...
programação concorrente por si própria é difícil e propensa a erros \gerosa{não entendi}.
...
provedor de infraestruturas e a linguagem de programação da aplicação \gerosa{?}.
...
pode-se \gerosa{aplicar mais facilmente a melhoria (?)} para diferentes aplicações utilizando essa
...
Para empoderar \gerosa{está certo ``empoderar''?} tais sistemas, o EE fornece informação e controle
...
para aumentar seu desempenho \gerosa{melhorar parágrafo}.

Replicação de serviço é um tipo particular e importante de atualização de composição
\gerosa{ao replicar vc altera a composição? não é a mesma composição?}.
...
\gerosa{Fala no foco de pesquisadores. E practioners? Qual é o foco da ferramenta?}
