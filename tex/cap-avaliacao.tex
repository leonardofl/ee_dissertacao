...
Neste capítulo apresentaremos experimentos realizados com o Enactment Engine com objetivo de avaliar
as vantagens de seu uso em contraste com soluções \emph{ad-hoc} de implantação automatizada.
Também avaliaremos seu desempenho e escalabilidade para verificar a viabilidade de seu uso
...
Nós conduzimos experimentos para avaliar o desempenho e escalabilidade do
Enactment Engine em função de sua capacidade de implantar um número significante
de composições em uma plataforma de nuvem já utilizada no mercado.
...
Embora respostar não sejam representadas por questão de simplicidade,
...
Inicialmente, conduzimos uma análise multi-variada do desempenho do EE implantando composições nos seguintes cenários:
...
Em nossos experimentos, a política de alocação de nós foi o ``cria nós até um limite, e depois faz rodízio entre eles'',
...
Nós utilizamos a Amazon EC2 como provedor de infraestrutura,
com instâncias do tipo \emph{small},
cada uma com 1.7 GiB of RAM, uma vCPU com processamento
equivalente a 1.0--1.2 GHz, e rodando Ubuntu GNU/Linux 12.04.
...
Os resultados mostram que o EE escala bem em termos de serviços sendo implantados.
...
Durantes nossos experimentos, nós observamos que, graças aos mecanismos de tolerância a falhas do EE,
a quantidade de falhas na implantação foi baixa: todos os serviços foram corretamente implantados em mais
de 75\% das execuções.
Por ``falha'' nós dizemos quando um serviço não foi corretamente implantado.
...
Finalmente, nós observamos que 80\% das execuções não utilizaram a reserva de nós ociosos.
...
Nós também observamos que o tempo de implantação não foi significativamente afetado
...
Nós também conduzimos experimentos para avaliar o desempenho e escalabilidade do Enactment Engine
...
enquanto o ambiente utilizado para executar o EE foi uma máquina virtual (8 GiB de RAM e 4 vCPUs)
hospedada na infraestrutura de nossa Universidade.
Os nós criados eram instâncias \emph{small} da Amazon EC2 e o \emph{timeout} de criação dos nós era de 250 segundos.
...
Noque que até o maior tempo de implantação (cerca de 35 minutos para 1800 serviços)
pode ser considerado baixo se considerarmos o longo período pelo qual uma composição de serviços
permanece sem receber atualizações.
